package cn.fusiondb.common.internal

import cn.fusiondb.common.util.Utils
  ...
  private def initializeLogging(isInterpreter: Boolean, silent: Boolean): Unit = {
    ...
    if (Logging.isLog4j12()) {
      ...
      if (!log4j12Initialized) {
        Logging.defaultFDBLog4jConfig = true
        val defaultLogProps = "org/apache/spark/log4j-defaults.properties"
        Option(Utils.getFDBClassLoader.getResource(defaultLogProps)) match {
          case Some(url) =>
            PropertyConfigurator.configure(url)
            if (!silent) {
              System.err.println(s"Using FDB's default log4j profile: $defaultLogProps")
            }
          case None =>
            System.err.println(s"FDB was unable to load $defaultLogProps")
        }
      }
        ...
        if (replLevel != rootLogger.getEffectiveLevel()) {
          if (!silent) {
            ...
            System.err.println("To adjust logging level use sc.setLogLevel(newLevel). " +
              "For FDB use setLogLevel(newLevel).")
          }
...
private[fusiondb] object Logging {
  @volatile private var initialized = false
  @volatile private var defaultRootLevel: Level = null
  @volatile private var defaultFDBLog4jConfig = false
  ...
  def uninitialize(): Unit = initLock.synchronized {
    if (isLog4j12()) {
      if (defaultFDBLog4jConfig) {
        defaultFDBLog4jConfig = false
